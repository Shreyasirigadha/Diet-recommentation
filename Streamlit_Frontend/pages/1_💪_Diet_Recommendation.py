    ...
    def generate_recommendations(self,):
        ...
        for meal in self.meals_calories_perc:
            ...
            if "error" in response:
                recommended_recipes = []  # or handle error gracefully
            else:
...
class Display:
    ...
    def display_meal_choices(self, person, recommendations):    
        ...
        # Get meals in correct order from person object
        meals = list(person.meals_calories_perc.keys())
        cols = st.columns(len(meals))
        choices = []

        # Loop through each meal dynamically
        for meal_name, col, recs in zip(meals, cols, recommendations):
            with col:
                choice = st.selectbox(
                    f"Choose your {meal_name}:",
                    [recipe['Name'] for recipe in recs],
                    key=f"{meal_name}_choice"   # unique key avoids dropdown conflicts
                )
    ...
    # Graph 1: Calories comparison
        ...
        nutritions_graph_options = {
            ...
            "series": [{
                ...
                "data": [{"value": round(total_nutrition_values[val]), "name": val} for val in total_nutrition_values],
            }]
        }
